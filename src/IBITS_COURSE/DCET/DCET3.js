...
export default class DCET3 extends React.Component {
  constructor(props) {
    super(props);
  ...
  refreshList() {
    fetch(variables.API_URL + "dcet3")
      .then((response) => response.json())
      .then((data) => {
        this.setState({ dcet3: data });
      });
  }
  ...
  deleteClick(id) {
    if (window.confirm("Are you sure?")) {
      fetch(variables.API_URL + "dcet3/" + id, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
  ...
  viewClick(name) {
    ...
    fetch(variables.API_URL + "dcet3/" + name, {
      method: "GET",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
      ...
      .then((data) => {
        this.setState({ dcet3: data });
        console.log({ dcet3: data });
      });
  }

  render() {
    const { dcet3, Title, studentId, studentName, Gmail, DateOfJoining } =
      this.state;
    ...
    return (
      <div id="wtf">
        ...
        >
          Diploma Computer Engineering Technology (DCET-3)
        </h2>
        <button
          type="button"
          className="btn btn-primary m-2 float-end"
          data-bs-toggle="modal"
          data-bs-target="#sampleModal19"
