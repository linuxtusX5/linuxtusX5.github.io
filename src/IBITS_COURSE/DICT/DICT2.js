...
export default class DICT2 extends React.Component {
  constructor(props) {
    super(props);
  ...
  refreshList() {
    fetch(variables.API_URL + "dict2")
      .then((response) => response.json())
      .then((data) => {
        this.setState({ dict2: data });
      });
  }
  ...
  deleteClick(id) {
    if (window.confirm("Are you sure?")) {
      fetch(variables.API_URL + "dict2/" + id, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
  ...
  viewClick(name) {
    ...
    fetch(variables.API_URL + "dict2/" + name, {
      method: "GET",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
      ...
      .then((data) => {
        this.setState({ dict2: data });
        console.log({ dict2: data });
      });
  }

  render() {
    const { dict2, Title, studentId, studentName, Gmail, DateOfJoining } =
      this.state;
    ...
    return (
      <div id="wtf">
        ...
        >
          Diploma Information Communication Technology (DICT-2)
        </h2>
        <button
          type="button"
          className="btn btn-primary m-2 float-end"
          data-bs-toggle="modal"
          data-bs-target="#sampleModal11"
