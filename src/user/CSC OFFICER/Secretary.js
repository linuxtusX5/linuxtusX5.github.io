...
const Secretary = () => {
  ...
  const updateData = async (e) => {
    ...
    await db.collection("Secretary").doc(dataIdToBeUpdated).update({
      name: updatedInput,
  ...
  const onSubmit = async (e) => {
    ...
    await db.collection("Secretary").doc(username).set({
      name: username,
      EventPic: fileUrl,
      Description: desc,
      timestamp: new Date(),
  ...
  const deletedocs = (dataIdToBeUpdated) => {
    db.collection("Secretary").doc(dataIdToBeUpdated).delete();
    Swal.fire({
      position: "center",
      icon: "success",
      title: "Success",
        ...
        <DialogActions>
          ...
          <DrawerHeader />
            ...
            <div className="container header2">
              <h1>
                <span className="design2">SECRETARY</span>
              </h1>
            </div>
                {users.map((user, name) => {
                  return (
