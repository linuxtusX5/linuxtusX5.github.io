...
const Treasurer = () => {
  ...
  const updateData = async (e) => {
    ...
    await db.collection("Treasurer").doc(dataIdToBeUpdated).update({
      name: updatedInput,
  ...
  const onSubmit = async (e) => {
    ...
    await db.collection("Treasurer").doc(username).set({
      name: username,
      EventPic: fileUrl,
      Description: desc,
      timestamp: new Date(),
  ...
  useEffect(() => {
    const q = query(collection(db, "Treasurer"), orderBy("timestamp", "desc"));
  ...
  const deletedocs = (dataIdToBeUpdated) => {
    db.collection("Treasurer").doc(dataIdToBeUpdated).delete();
    Swal.fire({
      position: "center",
      icon: "success",
      title: "Success",
        ...
        <DialogActions>
          ...
          <DrawerHeader />
            ...
            <div className="container header2">
              <h1>
                <span className="design2">TREASURER</span>
              </h1>
            </div>
                {users.map((user, name) => {
                  return (
