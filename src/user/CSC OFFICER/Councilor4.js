...
const Councilor4 = () => {
  ...
  const updateData = async (e) => {
    ...
    await db.collection("Councilor4").doc(dataIdToBeUpdated).update({
      name: updatedInput,
  ...
  const onSubmit = async (e) => {
    ...
    await db.collection("Councilor4").doc(username).set({
      name: username,
      EventPic: fileUrl,
      Description: desc,
      timestamp: new Date(),
  ...
  const deletedocs = (dataIdToBeUpdated) => {
    db.collection("Councilor4").doc(dataIdToBeUpdated).delete();
    Swal.fire({
      position: "center",
      icon: "success",
      title: "Success",
