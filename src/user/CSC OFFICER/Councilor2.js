...
const Councilor2 = () => {
  ...
  const updateData = async (e) => {
    ...
    await db.collection("Councilor2").doc(dataIdToBeUpdated).update({
      name: updatedInput,
  ...
  const onSubmit = async (e) => {
    ...
    await db.collection("Councilor2").doc(username).set({
      name: username,
      EventPic: fileUrl,
      Description: desc,
      timestamp: new Date(),
  ...
  const deletedocs = (dataIdToBeUpdated) => {
    db.collection("Councilor2").doc(dataIdToBeUpdated).delete();
    Swal.fire({
      position: "center",
      icon: "success",
      title: "Success",
