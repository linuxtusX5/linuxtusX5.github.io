...
const President = () => {
  ...
  const updateData = async (e) => {
    ...
    await db.collection("President").doc(dataIdToBeUpdated).update({
      name: updatedInput,
  ...
  const onSubmit = async (e) => {
    ...
    await db.collection("President").doc(username).set({
      name: username,
      EventPic: fileUrl,
      Description: desc,
      timestamp: new Date(),
  ...
  useEffect(() => {
    const q = query(collection(db, "President"), orderBy("timestamp", "desc"));
  ...
  const deletedocs = (dataIdToBeUpdated) => {
    db.collection("President").doc(dataIdToBeUpdated).delete();
    Swal.fire({
      position: "center",
      icon: "success",
      title: "Success",
        ...
        <DialogActions>
          ...
          <DrawerHeader />
            ...
            <div className="container header2">
              <h1>
                <span className="design2">PRESIDENT</span>
              </h1>
            </div>
                {users.map((user, name) => {
                  return (
