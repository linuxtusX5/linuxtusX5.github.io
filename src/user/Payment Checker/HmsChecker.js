...
// moment().format("DD-MM-YYYY hh:mm:ss");
  ...
  useEffect(() => {
    db.collection("HMSPayment").onSnapshot((snapshot) => {
      setPay(
        snapshot.docs.map((doc) => ({
          id: doc.id,
          data: doc.data(),
  ...
  const handleSubmit = async (e) => {
    ...
    await db.collection("HMSPayment").doc(username).set({
      Date: DateString,
      FeesName: fname,
      Amount: Amount,
    });
  ...
  const updateStatus = async (e) => {
    ...
    await db.collection("HMS").doc(dataId).update({
      Status: status,
    });
    setStatus("");
  };
  ...
  const deleteData = (id) => {
    db.collection("HMSPayment").doc(id).delete();
  };

  const UpdatedPayment = async (e) => {
    e.preventDefault();
    handleClose4();
    await db.collection("HMSPayment").doc(dataId2).update({
    ...
    <>
      ...
      <div className="App">
        ...
        >
          ...
          >
            <strong style={{ marginTop: "20px" }}>HMS Payment Checker</strong>
          </h1>
        </Card>
