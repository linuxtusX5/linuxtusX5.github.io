...
function PasoaChecker() {
  ...
  useEffect(() => {
    db.collection("PASOAPayment").onSnapshot((snapshot) => {
      setPay(
        snapshot.docs.map((doc) => ({
          id: doc.id,
          data: doc.data(),
  ...
  const handleSubmit = async (e) => {
    ...
    await db.collection("PASOAPayment").doc(username).set({
      Date: DateString,
      FeesName: fname,
      Amount: Amount,
    });
  ...
  const updateStatus = async (e) => {
    ...
    await db.collection("PASOA").doc(dataId).update({
      Status: status,
    });
    setStatus("");
  };
  ...
  const deleteData = (id) => {
    db.collection("PASOAPayment").doc(id).delete();
  };

  const UpdatedPayment = async (e) => {
    e.preventDefault();
    handleClose4();
    await db.collection("PASOAPayment").doc(dataId2).update({
    ...
    <>
      ...
      <div className="App">
        ...
        >
          ...
          >
            <strong style={{ marginTop: "20px" }}>PASOA Payment Checker</strong>
          </h1>
        </Card>
