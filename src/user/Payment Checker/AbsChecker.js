...
function AbsChecker() {
  ...
  useEffect(() => {
    db.collection("ABSPayment").onSnapshot((snapshot) => {
      setPay(
        snapshot.docs.map((doc) => ({
          id: doc.id,
          data: doc.data(),
  ...
  const handleSubmit = async (e) => {
    ...
    await db.collection("ABSPayment").doc(username).set({
      Date: DateString,
      FeesName: fname,
      Amount: Amount,
    });
    ...
    setAmount("");
          ...
          const updateStatus = async (e) => {
            ...
            await db.collection("ABS").doc(dataId).update({
              Status: status,
            });
            setStatus("");
          };
  ...
  const deleteData = (id) => {
    db.collection("ABSPayment").doc(id).delete();
  };
...
const UpdatedPayment = async (e) => {
  e.preventDefault();
  handleClose4()
     await db.collection("ABSPayment").doc(dataId2).update({
