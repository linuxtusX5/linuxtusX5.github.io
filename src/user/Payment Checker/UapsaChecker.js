...
function UapsaChecker() {
  ...
  useEffect(() => {
    db.collection("UAPSAPayment").onSnapshot((snapshot) => {
      setPay(
        snapshot.docs.map((doc) => ({
          id: doc.id,
          data: doc.data(),
  ...
  const handleSubmit = async (e) => {
    ...
    await db.collection("UAPSAPayment").doc(username).set({
      Date: DateString,
      FeesName: fname,
      Amount: Amount,
    });
  ...
  const updateStatus = async (e) => {
    ...
    await db.collection("UAPSA").doc(dataId).update({
      Status: status,
    });
    setStatus("");
  };
  ...
  const deleteData = (id) => {
    db.collection("UAPSAPayment").doc(id).delete();
  };

  const UpdatedPayment = async (e) => {
    e.preventDefault();
    handleClose4();
    await db.collection("UAPSAPayment").doc(dataId2).update({
    ...
    <>
      ...
      <div className="App">
        ...
        >
          ...
          >
            <strong style={{ marginTop: "20px" }}>UAPSA Payment Checker</strong>
          </h1>
        </Card>
