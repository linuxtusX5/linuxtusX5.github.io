...
function YesChecker() {
  ...
  useEffect(() => {
    db.collection("YESPayment").onSnapshot((snapshot) => {
      setPay(
        snapshot.docs.map((doc) => ({
          id: doc.id,
          data: doc.data(),
  ...
  const handleSubmit = async (e) => {
    ...
    await db.collection("YESPayment").doc(username).set({
      Date: DateString,
      FeesName: fname,
      Amount: Amount,
    });
  ...
  const updateStatus = async (e) => {
    ...
    await db.collection("YES").doc(dataId).update({
      Status: status,
    });
    setStatus("");
  };
  ...
  const deleteData = (id) => {
    db.collection("YESPayment").doc(id).delete();
  };

  const UpdatedPayment = async (e) => {
    e.preventDefault();
    handleClose4();
    await db.collection("YESPayment").doc(dataId2).update({
    ...
    <>
      ...
      <div className="App">
        ...
        >
          ...
          >
            <strong style={{ marginTop: "20px" }}>YES Payment Checker</strong>
          </h1>
        </Card>
