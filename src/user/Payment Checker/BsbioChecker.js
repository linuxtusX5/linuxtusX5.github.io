...
function BsbioChecker() {
  ...
  useEffect(() => {
    db.collection("BSBIOPayment").onSnapshot((snapshot) => {
      setPay(
        snapshot.docs.map((doc) => ({
          id: doc.id,
          data: doc.data(),
  ...
  const handleSubmit = async (e) => {
    ...
    await db.collection("BSBIOPayment").doc(username).set({
      Date: DateString,
      FeesName: fname,
      Amount: Amount,
    });
  ...
  const updateStatus = async (e) => {
    ...
    await db.collection("BSBIO").doc(dataId).update({
      Status: status,
    });
    setStatus("");
  };
  ...
  const deleteData = (id) => {
    db.collection("BSBIOPayment").doc(id).delete();
  };

  const UpdatedPayment = async (e) => {
    e.preventDefault();
    handleClose4();
    await db.collection("BSBIOPayment").doc(dataId2).update({
    ...
    <>
      ...
      <div className="App">
        ...
        >
          ...
          >
            <strong style={{ marginTop: "20px" }}>BSBIO Payment Checker</strong>
          </h1>
        </Card>
