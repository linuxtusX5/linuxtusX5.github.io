...
export default class BSIT3 extends React.Component {
  constructor(props) {
    super(props);
  ...
  refreshList() {
    fetch(variables.API_URL + "bsit3")
      .then((response) => response.json())
      .then((data) => {
        this.setState({ bsit3: data });
      });
  }
  ...
  deleteClick(id) {
    if (window.confirm("Are you sure?")) {
      fetch(variables.API_URL + "bsit3/" + id, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
  ...
  viewClick(name) {
    ...
    fetch(variables.API_URL + "bsit3/" + name, {
      method: "GET",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  render() {
    const { bsit3, Title, studentId, studentName, Gmail, DateOfJoining } =
      this.state;
