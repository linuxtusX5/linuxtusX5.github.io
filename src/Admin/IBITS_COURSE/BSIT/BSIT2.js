...
export default class BSIT2 extends React.Component {
  constructor(props) {
    super(props);
  ...
  refreshList() {
    fetch(variables.API_URL + "bsit2")
      .then((response) => response.json())
      .then((data) => {
        this.setState({ bsit2: data });
      });
  }
  onChange = (e) => {
    e.preventDefault();
  ...
  editClick(bSIT1) {
    this.setState({
      ...
      studentId: bSIT1.studentId,
      studentName: bSIT1.studentName,
      Gmail: bSIT1.Gmail,
      DateOfJoining: bSIT1.DateOfJoining,
    });
  }
  ...
  deleteClick(id) {
    if (window.confirm("Are you sure?")) {
      fetch(variables.API_URL + "bsit2/" + id, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
  ...
  viewClick(name) {
    ...
    fetch(variables.API_URL + "bsit2/" + name, {
      method: "GET",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  render() {
    const { bsit2, Title, studentId, studentName, Gmail, DateOfJoining } =
      this.state;
    ...
    return (
      <div id="wtf">
        ...
        <Table striped bordered hover variant="dark">
          ...
          <tbody>
            {bsit2.map((bSIT1) => (
              <tr key={bSIT1.studentId}>
                <td>{bSIT1.studentId}</td>
                <td>{bSIT1.studentName}</td>
                <td>{bSIT1.Gmail}</td>
                <td>{bSIT1.DateOfJoining}</td>
                <td>
                  <button
                    type="button"
                    className="btn btn-light mr-1"
                    data-bs-toggle="modal"
                    data-bs-target="#sampleModal3"
                    onClick={() => this.editClick(bSIT1)}
                    style={{
                      marginRight: 20,
                    }}
                  >
                  ...
                  <button
                    type="button"
                    className="btn btn-light mr-1"
                    onClick={() => this.deleteClick(bSIT1.studentId)}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
