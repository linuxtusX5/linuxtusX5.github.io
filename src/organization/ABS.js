...
export default class ABS extends React.Component{
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'abs')
        .then(response => response.json())
        .then(data => {
            this.setState({abs:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'abs/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
        ...
        <h1 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>AGRIBUSINESS SOCIETY (ABS)</h1>
            ...
            data-bs-toggle="modal" data-bs-target="#sampleModal" onClick={() => this.addClick()}>
                ...
                <Table striped bordered hover variant="dark">
                    ...
                    <tbody>
                        {abs.map(aBS => 
                        <tr key={aBS.studentId}>
                                <td>{aBS.studentId}</td>
                                <td>{aBS.studentName}</td>
                                <td>{aBS.Gmail}</td>
                                <td>{aBS.DateOfJoining}</td>
                                <td className='d-flex justify-content-center'>

                                    <button type='button' className='btn btn-light mr-1'
                                    onClick={() => this.deleteClick(aBS.studentId)}>
