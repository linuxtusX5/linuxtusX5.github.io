...
export default class HMS extends React.Component{
    constructor(props){
        super(props);
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'hms')
        .then(response => response.json())
        .then(data => {
            this.setState({hms:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'hms/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
        ...
        <h2 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>HOSPITALITY MANAGEMENT SOCIETY (HMS)</h2>
            ...
            data-bs-toggle="modal" data-bs-target="#sampleModal" onClick={() => this.addClick()}>
                ...
                <Table striped bordered hover variant="dark">
                    ...
                    <tbody>
                        {hms.map(hMS => 
                        <tr key={hMS.studentId}>
                                <td>{hMS.studentId}</td>
                                <td>{hMS.studentName}</td>
                                <td>{hMS.Gmail}</td>
                                <td>{hMS.DateOfJoining}</td>
                                <td className='d-flex justify-content-center'>
                                    <button type='button' className='btn btn-light mr-1'
                                    onClick={() => this.deleteClick(hMS.studentId)}>
