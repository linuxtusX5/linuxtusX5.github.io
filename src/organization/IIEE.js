...
export default class IIEE extends React.Component{
    constructor(props){
        super(props);
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'iiee')
        .then(response => response.json())
        .then(data => {
            this.setState({iiee:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'iiee/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
    ...
    return (
        <div id='wtf'>
        <h1 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>INSTITUTE OF INTEGRATED ELECTRICAL ENGINEERS (IIEE)</h1>
            ...
            data-bs-toggle="modal" data-bs-target="#sampleModal" onClick={() => this.addClick()}>
                ...
                <Table striped bordered hover variant="dark">
                    ...
                    <tbody>
                        {iiee.map(iIEE => 
                        <tr key={iIEE.studentId}>
                                <td>{iIEE.studentId}</td>
                                <td>{iIEE.studentName}</td>
                                <td>{iIEE.Gmail}</td>
                                <td>{iIEE.DateOfJoining}</td>
                                <td className='d-flex justify-content-center'>
                                    <button type='button' className='btn btn-light mr-1'
                                    onClick={() => this.deleteClick(iIEE.studentId)}>
