...
export default class YES extends React.Component{
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'yes')
        .then(response => response.json())
        .then(data => {
            this.setState({yes:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'yes/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
    ...
    return (
        <div id='wtf'>
        <h1 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>YOUNG EDUCATORS SOCIETY (YES)</h1>
            ...
            data-bs-toggle="modal" data-bs-target="#sampleModal" onClick={() => this.addClick()}>
                ...
                <Table striped bordered hover variant="dark">
                    ...
                    <tbody>
                        {yes.map(yES => 
                        <tr key={yES.studentId}>
                                <td>{yES.studentId}</td>
                                <td>{yES.studentName}</td>
                                <td>{yES.Gmail}</td>
                                <td>{yES.DateOfJoining}</td>
                                <td className='d-flex justify-content-center'>
                                    <button type='button' className='btn btn-light mr-1'
                                    onClick={() => this.deleteClick(yES.studentId)}>
