...
export default class SYNERTECH extends React.Component{
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'synertech')
        .then(response => response.json())
        .then(data => {
            this.setState({synertech:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'synertech/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
    ...
    return (
        <div id='wtf'>
        <h1 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>SYNERGISM OF STUDENT IN TECHNOLOGY (synertech)</h1>
            ...
            data-bs-toggle="modal" data-bs-target="#sampleModal" onClick={() => this.addClick()}>
                ...
                <Table striped bordered hover variant="dark">
                    ...
                    <tbody>
                        {synertech.map(sYNERTECH => 
                        <tr key={sYNERTECH.studentId}>
                                <td>{sYNERTECH.studentId}</td>
                                <td>{sYNERTECH.studentName}</td>
                                <td>{sYNERTECH.Gmail}</td>
                                <td>{sYNERTECH.DateOfJoining}</td>
                                <td className='d-flex justify-content-center'>
                                    <button type='button' className='btn btn-light mr-1'
                                    onClick={() => this.deleteClick(sYNERTECH.studentId)}>
