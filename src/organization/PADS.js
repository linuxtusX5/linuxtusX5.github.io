...
export default class PADS extends React.Component{
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'pads')
        .then(response => response.json())
        .then(data => {
            this.setState({pads:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'pads/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
    ...
    return (
        <div id='wtf'>
        <h1 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>PUBLIC ADMINISTRATION SOCIETY (PADS)</h1>
            ...
            data-bs-toggle="modal" data-bs-target="#sampleModal" onClick={() => this.addClick()}>
                ...
                <Table striped bordered hover variant="dark">
                    ...
                    <tbody>
                        {pads.map(pADS => 
                        <tr key={pADS.studentId}>
                                <td>{pADS.studentId}</td>
                                <td>{pADS.studentName}</td>
                                <td>{pADS.Gmail}</td>
                                <td>{pADS.DateOfJoining}</td>
                                <td className='d-flex justify-content-center'>
                                    <button type='button' className='btn btn-light mr-1'
                                    onClick={() => this.deleteClick(pADS.studentId)}>
