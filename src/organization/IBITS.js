...
export default class IBITS extends React.Component{
    constructor(props){
        super(props);
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'ibits')
        .then(response => response.json())
        .then(data => {
            this.setState({ibits:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'ibits/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
    ...
    return (
        <div id='wtf'>
        <h2 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>INSTITUTE OF BRILLIANT INFORMATION TECHNOLOGY STUDENT (IBITS)</h2>
