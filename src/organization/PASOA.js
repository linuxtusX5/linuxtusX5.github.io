...
export default class PASOA extends React.Component{
    constructor(props){
        super(props);
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'pasoa')
        .then(response => response.json())
        .then(data => {
            this.setState({pasoa:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'pasoa/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
    ...
    return (
        <div id='wtf'>
        <h2 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>PHILIPPINE ASSOCIATION OF STUDENT IN OFFICE ADMINISTRATION (PASOA)</h2>
            ...
            data-bs-toggle="modal" data-bs-target="#sampleModal" onClick={() => this.addClick()}>
                ...
                <Table striped bordered hover variant="dark">
                    ...
                    <tbody>
                        {pasoa.map(pASOA => 
                        <tr key={pASOA.studentId}>
                                <td>{pASOA.studentId}</td>
                                <td>{pASOA.studentName}</td>
                                <td>{pASOA.Gmail}</td>
                                <td>{pASOA.DateOfJoining}</td>
                                <td className='d-flex justify-content-center'>
                                    <button type='button' className='btn btn-light mr-1'
                                    onClick={() => this.deleteClick(pASOA.studentId)}>
