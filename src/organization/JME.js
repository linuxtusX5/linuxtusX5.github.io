...
export default class JME extends React.Component{
    constructor(props){
        super(props);
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'jme')
        .then(response => response.json())
        .then(data => {
            this.setState({jme:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'jme/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
    ...
    return (
        <div id='wtf'>
        <h1 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>JUNIOR MARKETING EXECUTIVE (JME)</h1>
            ...
            data-bs-toggle="modal" data-bs-target="#sampleModal" onClick={() => this.addClick()}>
                ...
                <Table striped bordered hover variant="dark">
                    ...
                    <tbody>
                        {jme.map(jME => 
                        <tr key={jME.studentId}>
                                <td>{jME.studentId}</td>
                                <td>{jME.studentName}</td>
                                <td>{jME.Gmail}</td>
                                <td>{jME.DateOfJoining}</td>
                                <td className='d-flex justify-content-center'>
                                    <button type='button' className='btn btn-light mr-1'
                                    onClick={() => this.deleteClick(jME.studentId)}>
