...
export default class UAPSA extends React.Component{
    constructor(props){
        super(props);
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'uapsa')
        .then(response => response.json())
        .then(data => {
            this.setState({uapsa:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'uapsa/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
    ...
    return (
        <div id='wtf'>
        <h2 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>UNITED ARCHITECTS OF THE PHILIPPINE STUDENT AUXILIARY (UAPSA)</h2>
            ...
            data-bs-toggle="modal" data-bs-target="#sampleModal" onClick={() => this.addClick()}>
                ...
                <Table striped bordered hover variant="dark">
                    ...
                    <tbody>
                        {uapsa.map(uAPSA => 
                        <tr key={uAPSA.studentId}>
                                <td>{uAPSA.studentId}</td>
                                <td>{uAPSA.studentName}</td>
                                <td>{uAPSA.Gmail}</td>
                                <td>{uAPSA.DateOfJoining}</td>
                                <td className='d-flex justify-content-center'>
                                    <button type='button' className='btn btn-light mr-1'
                                    onClick={() => this.deleteClick(uAPSA.studentId)}>
