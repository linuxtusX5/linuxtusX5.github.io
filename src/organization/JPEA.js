...
export default class JPEA extends React.Component{
    constructor(props){
        super(props);
    ...
    refreshList(){
         ...
         fetch(variables.API_URL + 'jpea')
        .then(response => response.json())
        .then(data => {
            this.setState({jpea:data});
        })
    }
    ...
    deleteClick(id){
        if(window.confirm("Are you sure?")){
            fetch(variables.API_URL + 'jpea/' + id,{
                method: 'DELETE',
                headers: {
                    'Accept':'application/json',
                    'Content-Type':'application/json'
    ...
    return (
        <div id='wtf'>
        <h1 className='d-flex justify-content-center mt-3' style={{color:'#ff0', padding: '10px'}}>JUNIOR PHILIPPINE INSTITUTE OF ACCOUNTANTS (jpia)</h1>
            ...
            data-bs-toggle="modal" data-bs-target="#sampleModal" onClick={() => this.addClick()}>
                ...
                <Table striped bordered hover variant="dark">
                    ...
                    <tbody>
                        {jpea.map(jPEA => 
                        <tr key={jPEA.studentId}>
                                <td>{jPEA.studentId}</td>
                                <td>{jPEA.studentName}</td>
                                <td>{jPEA.Gmail}</td>
                                <td>{jPEA.DateOfJoining}</td>
                                <td className='d-flex justify-content-center'>
                                    <button type='button' className='btn btn-light mr-1'
                                    onClick={() => this.deleteClick(jPEA.studentId)}>
